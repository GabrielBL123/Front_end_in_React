...
const AvaliacaoDetalhe = () => {
  ...
  const gerarLinkAvaliacao = async () => {
    try {
      ...
      const response = await axios.post(
        "/avaliacoes-mensais/avaliacao/gerar-link",
  ...
  const baixarRelatorioExcel = async () => {
    ...
    try {
      ...
      link.setAttribute(
        "download",
        `Relatorio_Avaliacao_${tokenAvaliacaoAtual}.xlsx`,
      );
  ...
  if (error) {
    return (
      ...
      >
        <div
          className="psy-shell max-w-xl w-full p-8 rounded-3xl text-center shadow-lg"
          style={{ backgroundColor: "#FCFBF7", border: "1px solid #DCD9CC" }}
        >
          <p className="font-medium text-lg mb-6" style={{ color: "#8A3E31" }}>
            {error}
          </p>
          <button
            onClick={() => navigate(-1)}
      ...
      <div className="w-full max-w-6xl mx-auto space-y-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          >
            <div>
              <h2
                className="psy-display text-2xl mb-6 pb-3 border-b"
                style={{ color: "#1E3835", borderBottomColor: "#E4E1D3" }}
              >
              ...
              <ul className="space-y-4">
                <li className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                  <span
                    className="font-semibold text-sm uppercase tracking-wider min-w-[120px]"
                    style={{ color: "#8B9188" }}
                  >
                    Criado em:
                  </span>
                  <span
                    className="font-medium px-3 py-2 rounded-xl flex-1 flex items-center"
                    style={{ backgroundColor: "#EDEEE8", color: "#1F2A27" }}
                  >
            ...
            >
              ...
              {linkCopiado && (
                <p
                  className="mt-3 break-all font-mono text-xs p-3 rounded-xl border"
                  style={{
                    backgroundColor: "#E3F0E6",
                    color: "#2F5C3E",
                    borderColor: "rgba(110,143,118,0.3)",
                  }}
                >
          ...
          >
            ...
            <ul className="space-y-3 flex-1">
              <li className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                <span
                  className="font-semibold text-sm uppercase tracking-wider min-w-[100px]"
                  style={{ color: "#8B9188" }}
                >
                  Nome:
                </span>
                <span
                  className="font-medium break-all px-3 py-2 rounded-xl flex-1 flex items-center"
                  style={{ backgroundColor: "#EDEEE8", color: "#1F2A27" }}
                >
              ...
              <li className="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-4">
                ...
                <span
                  className="font-medium break-all px-3 py-2 rounded-xl flex-1 flex items-center font-mono text-sm"
                  style={{ backgroundColor: "#EDEEE8", color: "#1F2A27" }}
                >
        ...
        <div
          className="psy-shell rounded-3xl p-8 shadow-sm"
          style={{ backgroundColor: "#FCFBF7", border: "1px solid #DCD9CC" }}
        >
          <h2
            className="psy-display text-2xl mb-6"
            style={{ color: "#1E3835" }}
          >
            Setores Cadastrados
          </h2>
          ...
          {avaliacao.funcionario?.length > 0 ? (
            <div className="space-y-8">
              {Object.entries(
                ...
                >
                  <div
                    className="p-4 font-bold text-lg flex justify-between items-center text-white"
                    style={{
            ...
            <div
              ...
              style={{ borderColor: "#D8D6CB", backgroundColor: "#EDEEE8" }}
            >
              <p className="font-medium text-sm" style={{ color: "#6B7570" }}>
                Nenhum funcionário respondeu ao questionário desta competência
                até o momento.
              </p>
            </div>
          )}
