...
const Questionario = () => {
  ...
  const { token } = useParams(); // ✨ Extrai o token gerado da URL
  ...
  useEffect(() => {
    const buscarSetoresDaAvaliacao = async () => {
      try {
        const response = await axios.get(`/avaliacoes-mensais/${token}`);
        
        // Pega os dados com base na estrutura do seu backend ({ message, data })
        const avaliacao = response.data.data || response.data; 

        if (avaliacao.empresa && avaliacao.empresa.setores) {
          setSetores(avaliacao.empresa.setores);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await axios.post(`/resposta/${token}`, JSON.stringify(payloadDTO), {
            ...
            <div className="flex flex-col">
              ...
              >
                <option value="" disabled className="text-gray-500">
                  {setores.length > 0 ? "Selecione o setor..." : "Carregando setores..."}
                </option>
                
                {setores.map((setor, idx) => (
                  <option 
                    key={setor.id || idx} 
                    value={setor.nome || setor.nomeSetor} 
