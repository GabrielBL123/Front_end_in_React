...
const CriaSetores = () => {
  ...
  const buscarSetores = async () => {
    try {
      ...
      const setoresDaEmpresa = todosSetores.filter(s => s.empresaId === auth?.empresaId);
  ...
  return (
    <div className="w-full max-w-5xl bg-white p-8 md:p-14 rounded-3xl shadow-2xl my-8 mx-auto">
      ...
      <div className="flex justify-between items-center mb-10 border-b-2 border-green-100 pb-5">
        <div>
          ...
          <p className="text-gray-500 mt-1 text-base">
            Cadastre ou remova setores da empresa <span className="font-bold text-gray-700">{auth?.empresaNome}</span>
          </p>
        </div>
        <button 
      ...
      <form onSubmit={handleSalvar} className="bg-gray-50 p-6 md:p-8 rounded-2xl border border-gray-200 mb-12 flex flex-col md:flex-row items-end gap-6 shadow-inner">
