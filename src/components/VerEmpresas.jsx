  ...
  return (
    ...
    >
      ...
      <div className="w-full max-w-7xl">
        {carregando && (
          <div className="psy-shell w-full p-12 text-center rounded-3xl" style={{ backgroundColor: "#FCFBF7", border: "1px solid #DCD9CC" }}>
            <div className="w-12 h-12 border-4 border-t-[#6E8F76] rounded-full animate-spin mx-auto mb-4" style={{ borderColor: "#E3F0E6", borderTopColor: "#6E8F76" }}></div>
        ...
        {erro && (
          <div className="psy-shell w-full p-6 rounded-2xl mb-8 text-center" style={{ backgroundColor: "rgba(201,123,107,0.1)", border: "1px solid rgba(201,123,107,0.4)" }}>
            <p className="font-bold text-lg mb-1" style={{ color: "#8A3E31" }}>⚠️ Atenção</p>
            <p style={{ color: "#8A3E31" }}>{erro}</p>
          </div>
        )}

        {!carregando && !erro && empresas.length === 0 && (
          <div className="psy-shell w-full p-16 text-center rounded-3xl" style={{ backgroundColor: "#FCFBF7", border: "1px dashed #DCD9CC" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {empresas.map((empresa, index) => (
              ...
              >
                ...
                <div className="absolute top-0 left-0 w-full h-2" style={{ backgroundColor: "#6E8F76" }}></div>

                <div className="flex-grow mt-2">
                  <h2 className="psy-display text-2xl mb-5" style={{ color: "#1E3835" }}>
                    {empresa.nome || empresa.nomeEmpresa || "Nome não informado"}
                  </h2>
                  
                  <div className="space-y-3 mb-6">
                    <div className="flex flex-col">
                      <span className="text-xs uppercase tracking-wider font-bold mb-1" style={{ color: "#8B9188" }}>CNPJ</span>
                      <span className="font-medium" style={{ color: "#3A423E" }}>{empresa.cnpj || "Não informado"}</span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs uppercase tracking-wider font-bold mb-1" style={{ color: "#8B9188" }}>E-mail</span>
                      <span className="font-medium truncate" style={{ color: "#3A423E" }} title={empresa.email || empresa.emailEmpresa}>
                        {empresa.email || empresa.emailEmpresa || "Não informado"}
                      </span>
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs uppercase tracking-wider font-bold mb-1" style={{ color: "#8B9188" }}>Telefone</span>
                      <span className="font-medium" style={{ color: "#3A423E" }}>
                  ...
                  {/* Sessão de Setores */}
                  <div>
                    <h3 className="text-xs font-bold uppercase tracking-wider mb-3 flex items-center gap-2" style={{ color: "#5C7D63" }}>
                      Setores Cadastrados 
